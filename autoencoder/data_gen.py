...
# Get training data
store = pd.HDFStore('../data/April_wfs.h5','r')
raw = store['raw'] 
...
# Temp, clean training
# The excluded waveforms were blank...
# Need a new check here most likely...
...
def return_shifted(conv_training_wf, seed=0):
    random.seed(seed)
...
def get_data(number_of_wfs_to_get, wf_seed=0, noise_seed=0):
    np.random.seed(wf_seed)
    ...
    for i, wf in enumerate(y):
        offset_with_noise[i], y[i] = return_shifted(wf, seed=noise_seed)
