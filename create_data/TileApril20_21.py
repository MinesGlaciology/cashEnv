...
def resample_tif(tif_file, pixel_spacing=0.5):
    ...
    graster = gr.GeoRaster(dest.GetRasterBand(1).ReadAsArray(),
                           dest.GetGeoTransform(), #-32767,0,
                           projection = wgs84, datatype = gdal.GDT_Float32)
    # New, because georaster is now ignoring mask values...
    graster.raster.mask = graster.raster == 0.0
    return graster
...
input_files = "../april20/IODEM3*.tif"
